...
| ID | Concern | Code Surface | Doc Surface | Divergence | Why | Status | Exit Condition |
|---|---|---|---|---|---|---|---|
| DL-20260416-01 | architecture/domain | `agos_app/app/**` (runtime chưa có `ProjectScope`) | `docs/changelog/v1/adrs/ADR-013-projectscope-is-the-soft-value-stream-scope.md`, `docs/changelog/v1/architecture/03-domain-glossary.md`, `docs/changelog/v1/architecture/04-canonical-data-model.md`, `docs/changelog/v1/architecture/05-event-catalog.md`, `docs/changelog/v1/architecture/06-state-transitions.md`, `docs/changelog/v1/architecture/07-permission-matrix.md`, `docs/changelog/v1/architecture/10-assumptions-and-migration-path.md`, `docs/changelog/v1/diagram/03-canonical-data-model-erd.md`, `docs/changelog/v1/diagram/04-event-storming-event-map.md`, `docs/changelog/v1/diagram/08-role-based-view-permission-diagram.md` | Docs hiện đã khóa `ProjectScope`, assignment, contribution, shared-resource, và project economics baseline trước khi runtime schema/routes/services/events được rollout. | PR-0 khóa architecture decision trước để các PR schema/write-path/reporting sau bám đúng authority docs thay vì thiết kế ngược từ implementation tạm thời. | open | Resolved when `ProjectScope` aggregate, assignment lane, và ít nhất một project reporting slice được implement, hoặc khi ADR-013 bị supersede. |
| DL-20260415-01 | architecture/domain | `agos_app/app/**`, `agos_app/alembic/**` | `docs/changelog/v1/adrs/ADR-012-organization-is-legal-operating-owner-tenant-remains-deployment-boundary.md`, `docs/changelog/v1/architecture/03-domain-glossary.md`, `docs/changelog/v1/architecture/04-canonical-data-model.md`, `docs/changelog/v1/architecture/08-integration-contracts.md`, `docs/changelog/v1/architecture/10-assumptions-and-migration-path.md` | Runtime hiện đã có standalone `organizations` schema, CRUD, state actions, event emission, additive `organization_id` propagation cho canonical farm/commercial records, và order-driven `pending_fulfillment_board`; divergence còn lại là integration-facing surfaces cùng các deferred relationship/read-model slices như customer-affinity surfaces chưa rollout hết. | PR-1 khóa decision trước, các slice tiếp theo đang rollout dần thay vì mutate toàn bộ Phase 1 schema trong một bước. | open | Resolved when remaining integration-facing và deferred relationship/read-model `organization_id` rollout slices land, hoặc khi architecture decision bị supersede. |
