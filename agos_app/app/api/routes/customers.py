...
@router.get("", response_model=CustomerListResponse, responses={422: CUSTOMER_ERROR_RESPONSES[422]})
def list_customers(
    phone: str | None = None,
    q: str | None = None,
...
@router.get("/duplicate-candidates", response_model=CustomerDuplicateCandidateListResponse)
def list_duplicate_candidates() -> CustomerDuplicateCandidateListResponse:
    return CustomerDuplicateCandidateListResponse(items=svc.list_duplicate_candidates())


@router.get("/{customer_id}", response_model=CustomerDetail, responses={404: CUSTOMER_ERROR_RESPONSES[404], 422: CUSTOMER_ERROR_RESPONSES[422]})
def get_customer(customer_id: UUID) -> CustomerDetail:
    return svc.get_customer(str(customer_id))
...
@router.get("/{customer_id}/duplicate-candidates", response_model=CustomerDuplicateCandidateListResponse, responses={404: CUSTOMER_ERROR_RESPONSES[404], 422: CUSTOMER_ERROR_RESPONSES[422]})
def list_customer_duplicate_candidates(customer_id: UUID) -> CustomerDuplicateCandidateListResponse:
    return CustomerDuplicateCandidateListResponse(items=svc.list_customer_duplicate_candidates(str(customer_id)))
